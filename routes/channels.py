...
def check_plan_limit(user_id: str):
    ...
    if len(current.data) >= max_channels:
        raise HTTPException(status_code=403, detail=f"Plan limit! Max {max_channels} channels. Upgrade karo!")


class ChannelCreate(BaseModel):
    channel_name: str
    drive_folder_id: str = "pending"
...
async def create_channel(channel: ChannelCreate, authorization: str = Header(...)):
    ...
    return {"channel": result.data[0], "message": "Channel created!"}
...
async def load_videos(channel_id: str, authorization: str = Header(...)):
    ...
    return {"added": added, "message": f"{added} videos added!"}
