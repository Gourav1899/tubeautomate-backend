...
async def analytics_summary(authorization: str = Header(...)):
    ...
    return {"total_uploaded": len(done.data), "total_failed": len(failed.data), "total_views": total_views, "success_rate": success_rate}
...
async def analytics_per_channel(authorization: str = Header(...)):
    ...
    for ch in channels.data:
        ...
        result.append({"channel_name": ch["channel_name"], "total_uploaded": len(done.data), "total_views": sum(r["views"] for r in views.data)})
