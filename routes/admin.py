...
async def admin_stats(authorization: str = Header(...)):
    ...
    return {"total_users": len(users.data), "active_channels": len(channels.data), "total_uploads": len(uploads.data), "plan_breakdown": plan_breakdown}


@router.patch("/users/{user_id}/plan")
async def change_user_plan(user_id: str, plan: str, authorization: str = Header(...)):
    get_admin_user(authorization)
    if plan not in ["free", "starter", "pro", "agency"]:
