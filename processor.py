"""Video Processor — Drive → Metadata → Thumbnail → YouTube"""
import os
import logging
...
def process_one_video(channel: dict):
    ...
    try:
        ...
        meta_gen = MetadataGenerator(
            api_key=profile.get("openrouter_api_key") or os.environ.get("OPENROUTER_API_KEY"),
        ...
        thumb_maker = ThumbnailMaker(
            cloud_name=profile.get("cloudinary_cloud_name") or os.environ.get("CLOUDINARY_CLOUD_NAME"),
            api_key=profile.get("cloudinary_api_key") or os.environ.get("CLOUDINARY_API_KEY"),
            api_secret=profile.get("cloudinary_api_secret") or os.environ.get("CLOUDINARY_API_SECRET")
        )
        thumbnail_url = thumb_maker.create(video["video_file_id"], metadata["thumbnail_text"])
        ...
        video_path = watcher.download_video(video["video_file_id"], video["video_name"])
